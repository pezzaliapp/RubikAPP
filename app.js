...
const log = (...a)=>{ const s=a.map(x=>typeof x==='string'?x:JSON.stringify(x)).join(' '); diagEl.textContent += s+'\n'; console.log(...a); };
window.addEventListener('error', e=>{ log('window.onerror:', e.message); });

try{
  if(!window.THREE){ log('ERRORE: THREE non presente'); throw new Error('THREE non caricato (CDN bloccato?)'); }
  log('THREE r'+THREE.REVISION+' caricato');
  ...
  log('Renderer ok, size=', stage.clientWidth, stage.clientHeight);
  ...
  // History / notation minimal (keep features)
  ...
  // Simple playback for scramble
  function parseAlg(alg){ return alg.trim().split(/\s+/).filter(Boolean).map(t=>t.toUpperCase()); }
  ...
  document.getElementById('btn-reset').addEventListener('click', ()=>{
    ...
    document.getElementById('move-alg').textContent=''; document.getElementById('move-count').textContent='0';
    renderer.render(scene,camera); log('Reset done');
  });
  document.getElementById('btn-scramble').addEventListener('click', async ()=>{
    ...
    log('Scramble:', seq.join(' ')); await playAlg(seq.join(' '));
  });
  document.getElementById('btn-undo').addEventListener('click', ()=>{
    ...
    document.getElementById('move-alg').textContent = moveAlg.join(' ');
  ...
  document.getElementById('btn-copy').addEventListener('click', ()=>{
    const s = (document.getElementById('move-alg').textContent||'').trim(); navigator.clipboard?.writeText(s); log('Alg copiato:', s);
  });
  ...
  diagEl.textContent += 'ERRORE FATALE: ' + (err && err.message ? err.message : String(err)) + '\\n';
  console.error(err);
}
})();