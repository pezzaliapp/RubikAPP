(()=>{
const diagEl = document.getElementById('diag'); const log=(...a)=>{ console.log(...a); if(diagEl) diagEl.textContent += a.join(' ') + '\n'; };
...
document.getElementById('btn-scramble').addEventListener('click', async ()=>{
  ...
  await (async()=>{ for(const t of seq){ const base=t[0], mod=t.slice(1); const dbl=mod.includes('2'); const prime=mod.includes(\"'\")||mod.includes('’'); const reps=dbl?2:1; for(let r=0;r<reps;r++){ if(base==='U') await rotateLayer('Y', +1, prime?-1:+1, true); else if(base==='D') await rotateLayer('Y', -1, prime?+1:-1, true); else if(base==='R') await rotateLayer('X', +1, prime?+1:-1, true); else if(base==='L') await rotateLayer('X', -1, prime?-1:+1, true); else if(base==='F') await rotateLayer('Z', +1, prime?+1:-1, true); else if(base==='B') await rotateLayer('Z', -1, prime?-1:+1, true); addMoveNotation(base + (dbl? '2': (prime?\"'\":''))); } } })();
});
...
window.addEventListener('keydown', async (e)=>{
  ...
  if('UDLRFB'.includes(k)){
    ...
    const base=tok[0], mod=tok.slice(1); const dbl2=mod.includes('2'); const prime2=mod.includes(\"'\")||mod.includes('’');
    const reps=dbl2?2:1;
    for(let r=0;r<reps;r++){
      if(base==='U') await rotateLayer('Y', +1, prime2?-1:+1, true);
      else if(base==='D') await rotateLayer('Y', -1, prime2?+1:-1, true);
      else if(base==='R') await rotateLayer('X', +1, prime2?+1:-1, true);
      else if(base==='L') await rotateLayer('X', -1, prime2?-1:+1, true);
      else if(base==='F') await rotateLayer('Z', +1, prime2?+1:-1, true);
      else if(base==='B') await rotateLayer('Z', -1, prime2?-1:+1, true);
      addMoveNotation(base + (dbl2? '2': (prime2?\"'\":'')));
    }
  }
});
