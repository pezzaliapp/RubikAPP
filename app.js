...
renderer.outputColorSpace = THREE.SRGBColorSpace; // colori coerenti su iOS/Mac
...
function makeSticker(color, axis, sign){
  ...
  const m = new THREE.MeshBasicMaterial({
    ...
    depthTest: true,
    depthWrite: false,          // non sporcare lo Z-buffer
    polygonOffset: true,
    polygonOffsetFactor: -4,    // spingi davanti alla plastica
    polygonOffsetUnits:  -4
  });
  const s = new THREE.Mesh(g, m);
  s.renderOrder = 2;            // disegna dopo i cubetti
  ...
  for (let i=-1;i<=1;i++){
    for (let j=-1;j<=1;j++){
      for (let k=-1;k<=1;k++){
        ...
        // sticker sulle facce esterne (tutte e 6)
...
function hitsAt(cx,cy){
  ...
  // intersechiamo anche gli sticker, poi risaliamo al cubetto
...
async function rotateLayer(axisChar, layerCoord, sign){
  ...
  const pick = axisChar==='X' ? m=>nearly(m.userData.coord.x,layerCoord)
...
document.getElementById('btn-scramble').addEventListener('click', async ()=>{
  ...
  for (let i=0;i<20;i++){
    ...
    await new Promise(res=>{
      const iv=setInterval(()=>{ if(!rotating){ clearInterval(iv); res(); }}, 18);
      rotateFromGesture(fake,dx,dy);
    });
  }
});
